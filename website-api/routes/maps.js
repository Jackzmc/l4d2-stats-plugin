...
export default function(pool) {
    router.get('/', routeCache.cacheSeconds(120), async (req, res) => {
        const [rows] = await pool.query(`
            SELECT i.mapid as map_id, i.name as map_name, AVG(r.value) as avg_rating, COUNT(DISTINCT r.steamid) as num_ratings, COUNT(g.id) as games_played, AVG(g.duration) as avg_duration, MAX(g.date_end) as last_played_time
            FROM map_info i
