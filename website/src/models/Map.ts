 ...
 */
export async function getMapPlayCount(officialMapsOnly: boolean = false): Promise<MapCountEntry[]> {
    let flags = officialMapsOnly ? MapFlags.OfficialMap : MapFlags.None

    const [rows] = await db.query<RowDataPacket[]>(`
        SELECT map, map_info.name, COUNT(map) count 
        FROM stats_games 
        RIGHT JOIN map_info ON map_info.mapid = stats_games.map AND map_info.flags & ?
